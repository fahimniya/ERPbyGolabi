...
public class ResourceWrapper {
	...
	public Resource[] showFacilityResources() {
	...
	public Resource[] showFundingResources() {
		DBManagement db = new DBManagement();
		String query = db.generateSelectQuery("FUNDINGRESOURCE", new String[] { "*" }, null, null);
		ResultSet rs = db.getQuery(query);
		ArrayList<Resource> resources = new ArrayList<Resource>();	
		try {
			while (rs.next()) {
	...
	public HumanResource[] showHumanResources() {
		...
		try {
			while (rs.next()) {
				String q = db.generateSelectQuery("USER", new String[] { "*" },
						new String[] { rs.getString("USERNAME") }, new String[] { "USERNAME" });
				ResultSet temp = db.getQuery(q);
				if (!temp.next())
					break;
				humanResources.add(new HumanResource(
						new User(temp.getString("USERNAME"), temp.getString("PASSWORD"), temp.getString("NAME")),
